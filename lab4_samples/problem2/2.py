...
def md_run(size=3, timestep=0.001, nsteps=10000, temperature=300):
    ...
    pe = [float(e) for e in pe]
    ke = [float(e) for e in ke]
    energy = [float(e) for e in energy]
    temp = [float(t) for t in temp]
    press = [float(t) for t in press]
    msd = [float(t) for t in msd]
    ...
    return simtime, pe, ke, energy, temp, press, dens, msd, rdfs
...
def two():
    simtime, pe, ke, energy, temp, press, dens, msd, rdfs = md_run(size=8, timestep=0.001, nsteps=10000, temperature=2580)

    plt.figure()
    plt.plot(simtime, energy)
    plt.xlabel('Step')
    plt.ylabel('Energy (eV)')
    plt.savefig('2B_energy.png')

    plt.figure()
    plt.plot(simtime, pe)
    plt.xlabel('Step')
    plt.ylabel('Potential energy (eV)')
    plt.savefig('2B_pe.png')

    plt.figure()
    plt.plot(simtime, press)
    plt.xlabel('Step')
    plt.ylabel('Pressure')
    plt.savefig('2C_press.png')
