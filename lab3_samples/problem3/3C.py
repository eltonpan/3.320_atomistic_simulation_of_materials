...
def make_struc(a, c):
    ...
    :param a, c : Lattice parameter in angstrom of BCT lattice
...
def compute_energy(a, c, nk, ecut):
    """
    Make an input template and select potential and structure, and the path where to run
    """
    pseudopots = {'Cu': PseudoPotential(ptype='uspp', element='Cu',
                                        functional='LDA', name='Cu.pz-d-rrkjus.UPF'),
                  'Au': PseudoPotential(ptype='uspp', element='Au',
                                        functional='LDA', name='Au.pz-d-rrkjus.UPF'),
                  }
    struc = make_struc(a = a, c = c)
    kpts = Kpoints(gridsize=[nk, nk, nk], option='automatic', offset=False)
    dirname = 'CuAu_a_{}_c_{}_ecut_{}_nk_{}'.format(a, c, ecut, nk)
...
def lattice_scan():
    ...
    nk_unique_list = []
    for nk in nk_list:
        output = compute_energy(a = a, c = c, ecut=ecut, nk=nk)
    ...
    plt.savefig('3C_CuAu.png', dpi = 100)
    plt.show()
